...
Designed as the long-term memory layer for agents and coding assistants: `/ingest` what a project
or document says, `/search` it back when you need context.
...
```
                       ┌──────────────────────┐
  POST /ingest ───────▶│  chunk_text()        │  fixed-size windows + overlap
                       │  1000 chars / 200    │
                       └──────────┬───────────┘
                                  │ one call per chunk
                                  ▼
...
[Langfuse](https://langfuse.com) when keys are configured.
...
| `CHUNK_SIZE`          | `1000`                                      | characters per chunk — *currently not applied, see below* |
| `CHUNK_OVERLAP`       | `200`                                      | overlap between chunks — *currently not applied*     |
...
- **`repo_filter` does not work.** `/search` filters on `metadata->>'repo_name'`, but `/ingest`
  writes `repo_name` as a table column and does not include it in `metadata`, so filtered searches
  return nothing.
- **No schema management.** The `learnings` table has to be created by hand; there are no
  migrations and no bootstrap script.
- **`CHUNK_SIZE` / `CHUNK_OVERLAP` are ignored.** Chunking currently uses hardcoded defaults
  (1000 / 200) instead of the configured values.
